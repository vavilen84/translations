...
Итак, мы подошли к мотивации для разработки встроенной функции append. Он делает то же самое, что и наш пример Append, с эквивалентной эффективностью, но работает для любого типа среза.

Слабость Go заключается в том, что любые операции общего типа должны предоставляться во время выполнения. Когда-нибудь это может измениться, но сейчас, чтобы упростить работу со срезами, Go предоставляет встроенную универсальную функцию добавления. Он работает так же, как наша версия среза int, но для любого типа среза.

Помните, что поскольку заголовок среза всегда обновляется вызовом добавления, вам необходимо сохранить возвращенный срез после вызова. Фактически, компилятор не позволит вам вызвать append без сохранения результата.
...
Как должно быть ясно, пустой срез может увеличиваться (при условии, что он имеет ненулевую емкость), но нулевой срез не имеет массива для ввода значений и никогда не может увеличиваться, чтобы содержать хотя бы один элемент.

Тем не менее, нулевой срез функционально эквивалентен срезу нулевой длины, даже если он ни на что не указывает. Он имеет нулевую длину и может быть добавлен с выделением. В качестве примера посмотрите приведенный выше однострочник, который копирует фрагмент, добавляя его к нулевому фрагменту.
...
Важным следствием подобной конструкции срезов строк является то, что создание подстроки очень эффективно. Все, что нужно сделать, это создать заголовок строки из двух слов. Поскольку строка предназначена только для чтения, исходная строка и строка, полученная в результате операции среза, могут безопасно совместно использовать один и тот же массив.
